import { ArrowLeft, MapPin, Wifi, Snowflake, Refrigerator, Car, WashingMachine, Bed } from 'lucide-react'

const ICONS = { Wifi, Snowflake, Refrigerator, Car, WashingMachine }
      ...
      <div className="px-4 -mt-6 relative z-10">
        <div className="bg-white rounded-3xl card-shadow p-5 mb-4">
          ...
          <div className="border-t pt-4">
            <p className="text-xs text-muted mb-2 font-semibold uppercase tracking-wide">Pilihan Katil</p>
            <div className="space-y-2">
              {room.beds.map(bed => (
                <div key={bed.id} className={`flex items-center gap-2 p-2 rounded-xl ${bed.occupied ? 'bg-red-50 opacity-60' : 'bg-green-50'}`}>
                  ...
                  <span className={`text-sm font-medium ${bed.occupied ? 'text-red-500 line-through' : 'text-green-700'}`}>
                    {bed.name}
                  </span>
                  <span className={`text-xs ml-auto ${bed.occupied ? 'text-red-400' : 'text-green-600'}`}>
                    {bed.occupied ? 'Occupied' : 'Tersedia'}
                  </span>
                </div>
              ))}
            </div>
