      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between border-t pt-3">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 text-muted text-xs">
              <Home size={12} />
              {property.rooms.length} bilik
            </div>
            <div className="flex items-center gap-1 text-muted text-xs">
              <Bed size={12} />
              {kosongBeds}/{totalBeds} kosong
            </div>
          </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1">
          {property.rooms.slice(0, 3).map(room => (
            <span key={room.id} className={`text-xs px-2 py-0.5 rounded-full ${room.beds.some(b => !b.occupied) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {room.name.length > 12 ? room.name.substring(0, 12) + '...' : room.name}
            </span>
          ))}
          {property.rooms.length > 3 && (
            <span className="text-xs text-muted px-2 py-0.5">+{property.rooms.length - 3} lagi</span>
          )}
        </div>
      </div>
    </div>
