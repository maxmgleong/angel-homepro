...
import { ArrowLeft, Edit2, Trash2, Plus, Save, X, Upload, Home, Bed, Users } from 'lucide-react'

const ICONS = { Wifi: '📶', Snowflake: '❄️', Refrigerator: '🧊', Car: '🚗', WashingMachine: '🧺' }
const FACILITIES_LIST = ['wifi', 'ac', 'peti_sejuk', 'parking', 'mesin basuh']
  ...
  return (
    <div>
      ...
      <div
        ...
        className="border-2 border-dashed border-accent rounded-xl p-3 text-center cursor-pointer hover:border-primary transition-colors"
      >
        {preview ? (
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-primary mb-1">Nama Property</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-primary mb-1">Lokasi</label>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => handleChange('description', e.target.value)} rows={3}
              className="w-full border-2 border-accent rounded-xl px-4 py-2.5 text-sm focus:border-primary focus:outline-none resize-none" />
          </div>
          <ImageUpload value={form.image} onChange={v => handleChange('image', v)} label="Gambar Property" />
        </div>
        <button onClick={() => onSave(form)} className="btn-primary w-full mt-4 flex items-center justify-center gap-2">
...
function EditRoomModal({ room, onSave, onClose }) {
  ...
  function toggleFacility(f) {
    const has = form.facilities.includes(f)
    setForm(prev => ({
      ...prev,
  ...
  function addBed() {
    setForm(prev => ({
      ...prev,
      beds: [...prev.beds, { id: Date.now(), name: `Katil ${prev.beds.length + 1}`, occupied: false }]
    }))
  }

  function removeBed(id) {
    setForm(prev => ({ ...prev, beds: prev.beds.filter(b => b.id !== id) }))
  }

  function updateBed(id, name) {
    setForm(prev => ({
      ...prev,
      beds: prev.beds.map(b => b.id === id ? { ...b, name } : b)
    }))
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold text-primary">Edit Room</h2>
          <button onClick={onClose} className="text-muted hover:text-primary"><X size={20} /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-primary mb-1">Nama Bilik</label>
            <input value={form.name} onChange={e => handleChange('name', e.target.value)}
              className="w-full border-2 border-accent rounded-xl px-4 py-2.5 text-sm focus:border-primary focus:outline-none" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-primary mb-1">Harga (RM/sebulan)</label>
          ...
          <ImageUpload value={form.image} onChange={v => handleChange('image', v)} label="Gambar Bilik" />
          <div>
            <label className="block text-xs font-semibold text-primary mb-1">Fasiliti</label>
            <div className="flex flex-wrap gap-2">
              {FACILITIES_LIST.map(f => (
                <button key={f} onClick={() => toggleFacility(f)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${form.facilities.includes(f) ? 'bg-primary text-white border-primary' : 'border-accent text-muted hover:border-primary'}`}>
                  {ICONS[f]} {f.charAt(0).toUpperCase() + f.slice(1).replace('_', ' ')}
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-primary mb-2">Katil dalam Bilik</label>
            <div className="space-y-2">
              {form.beds.map((bed, i) => (
                <div key={bed.id} className="flex items-center gap-2">
                  ...
                  <button onClick={() => removeBed(bed.id)} className="text-red-500 hover:text-red-600 p-2">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
...
export default function AdminPanel({ properties, tenants, onSave, onBack }) {
  const [editProp, setEditProp] = useState(null)
  const [editRoom, setEditRoom] = useState(null)
  const [addRoomToProp, setAddRoomToProp] = useState(null)
  ...
  function saveRoom(updated) {
    const updatedProps = properties.map(prop => ({
      ...prop,
      rooms: prop.rooms.map(r => r.id === updated.id ? updated : r)
    }))
    onSave(updatedProps)
    setEditRoom(null)
  }
  ...
  function addRoom(propId) {
    ...
    const newRoom = {
      ...
      price: 500,
      status: 'kosong',
      description: 'Description bilik baru',
      image: prop.rooms[0]?.image || 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=600&q=80',
      facilities: ['wifi'],
      beds: [{ id: Date.now(), name: 'Katil 1', occupied: false }]
    }
    onSave(properties.map(p => p.id === propId ? { ...p, rooms: [...p.rooms, newRoom] } : p))
    setAddRoomToProp(null)
  }
  ...
  const totalRooms = properties.reduce((a, p) => a + p.rooms.length, 0)
      ...
      <div className="bg-primary px-4 pt-6 pb-8 rounded-b-3xl">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-white/15 rounded-2xl p-3 text-center">
            ...
            <p className="text-white text-2xl font-bold">{properties.length}</p>
          </div>
          <div className="bg-white/15 rounded-2xl p-3 text-center">
            <p className="text-white/80 text-xs">Jumlah Bilik</p>
            <p className="text-white text-2xl font-bold">{totalRooms}</p>
          </div>
          <div className="bg-white/15 rounded-2xl p-3 text-center">
            <p className="text-white/80 text-xs">Katil Kosong</p>
            <p className="text-white text-2xl font-bold">{kosongBeds}/{totalBeds}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 -mt-4 pb-6">
          ...
          <div key={prop.id} className="bg-white rounded-3xl card-shadow p-4 mt-4">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex gap-1">
                <button onClick={() => setEditProp(prop)} className="p-2 text-primary hover:bg-accent rounded-lg">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => deleteProperty(prop.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="border-t pt-3 space-y-2">
              {prop.rooms.map(room => (
                <div key={room.id} className="flex items-center gap-2 bg-accent rounded-xl p-2.5">
                  <img src={room.image || prop.image} alt={room.name}
                  ...
                  <button onClick={() => setEditRoom(room)} className="p-1.5 text-primary hover:bg-white rounded-lg">
                    <Edit2 size={14} />
                  </button>
                  <button onClick={() => deleteRoom(prop.id, room.id)} className="p-1.5 text-red-500 hover:bg-white rounded-lg">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
        ...
        <button onClick={() => {
          const newProp = {
            ...
            description: 'Description property baru',
            rooms: []
          }
